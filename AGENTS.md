...
- 组件解析必须 fail closed：最新正式 Backend 不兼容、缺 capability、manifest/hash/attestation 不一致时直接失败，不回退旧 Backend。支持 Protocol major 2 且 minor-compatible；前端 Python SDK 当前声明 `>=2.8.3,<3.0.0`，由独立 `frontend-protocol-lock.json` 精确约束；Backend Runtime 的 Protocol 则继续由组件锁和已验证资产独立精确约束。
