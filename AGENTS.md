...
- 版本源包括 `version.txt`、应用 package TOML、package fallback `__version__` 与 `repository.json`，必须由 release prepare 同步。桥接期发布资产精确为 Classic win64 ZIP 与 sidecar、`VibeOCRClassic-{version}-full.nupkg`、`VibeOCRClassic-win-Setup.exe` 与 sidecar、`VibeOCRClassic-win-Portable.zip`、`releases.win.json`、`component-lock.json`、`frontend-protocol-lock.json` 与 SPDX SBOM；额外资产 fail closed。
