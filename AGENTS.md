...
- 组件解析必须 fail closed：最新正式 Backend 不兼容、缺 capability、manifest/hash/attestation 不一致时直接失败，不回退旧 Backend。支持 Protocol major 2 且 minor-compatible；本仓 Python SDK 依赖仍精确 pin 2.0.0，运行时可绑定兼容的 2.x minor。
...
- Python 3.13/Ruff 行宽与选择集以 `pyproject.toml` 为准。仓库无自定义 Git hook，提交前仍需运行 Ruff/pytest/组件输入验证。
