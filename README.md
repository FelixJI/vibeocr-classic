...
- 失败或取消会保留本次操作结果及已验证的下载缓存；未完成的下载可能需要重新开始。
  按界面给出的原因与可用动作处理：重新读取安装计划重试、用“修复运行环境”修复损坏组件，
  或在“下载设置”调整来源后再试。Python 包索引用于安装运行环境依赖，模型源用于引擎首次
  准备模型；修改来源或组件选择后需要重新预览确认，来源修改只影响下一次操作。
