...
正式用户资产只有 `VibeOCRClassic-win-Portable.zip`；full nupkg 与 `releases.win.json` 是内置
...
5. release build 同时生成真实 0.0.1 与候选版本 Portable，通过 loopback feed 完成
   check/download/apply/restart，验证稳定 state、Runtime 复用、断网搬移与外写入审计；
