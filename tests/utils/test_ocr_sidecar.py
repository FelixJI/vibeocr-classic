...
from vibeocr.classic.ocr_sidecar import (
    compute_fingerprint,
    load_sidecar,
    mark_completed,
    mark_pages_saved,
...
def isolated_sidecar_directory(tmp_path, monkeypatch):
    default_sessions_dir = ocr_sidecar._sessions_dir
    monkeypatch.setattr(ocr_sidecar, "_sessions_dir", lambda: tmp_path / "sessions")
...
def test_sidecar_path_is_path_slug_under_backend_state(
    tmp_path, monkeypatch, isolated_sidecar_directory
):
...
def test_default_sidecar_directory_preserves_version_1_product_path(
    tmp_path, monkeypatch, isolated_sidecar_directory
):
    ...
    assert sidecar_path(str(pdf)).parent == (
        product_root / "data" / "backend" / "ocr_sessions"
    )
...
def test_mark_pages_saved_merges_into_existing(tmp_path, monkeypatch):
    ...
    f.write_bytes(b"abc")
    monkeypatch.setattr(
        "vibeocr.classic.ocr_sidecar._sessions_dir", lambda: tmp_path / "sessions"
    )
...
def test_growth_ok_false_when_baseline_missing(tmp_path):
    ...
    from vibeocr.classic.ocr_sidecar import _growth_ok
